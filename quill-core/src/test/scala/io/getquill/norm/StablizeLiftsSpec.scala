  ...
  "stablize lifts" - {
    "ScalarValueLift" in {
      ...
      stablized must matchPattern {
        case ScalarValueLift("scalarValue", "scalarValue", StablizeLifts.Token(0), _, `astQuat`) =>
      }
    ...
    "multiple lifts" in {
      ...
      stablized must matchPattern {
        case BinaryOperation(
          ScalarValueLift("a", "a", StablizeLifts.Token(0), _, `quatA`),
          StringOperator.`+`, ScalarValueLift("b", "b", StablizeLifts.Token(1), _, `quatB`)) =>
      }

...
object CanDoBatchedInsert {
  def apply(ast: Ast, numRows: Int, idiom: Idiom, naming: NamingStrategy, isReturning: Boolean, transpileContext: TranspileContext): Boolean = {
    ...
    // only one action allowed per-query in general
      ...
      val statement = idiom.translate(actions.head, Quat.Unknown, ExecutionType.Unknown, transpileContext)(naming)._2

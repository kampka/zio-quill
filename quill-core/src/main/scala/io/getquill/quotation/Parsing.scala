...
trait Parsing extends ValueComputation with QuatMaking {
  ...
  val liftParser: Parser[Lift] = Parser[Lift] {
    ...
    case q"$pack.liftScalar[$t]($value)($encoder)"          => ScalarValueLift(value.toString, value.toString, value, encoder, inferQuat(q"$t".tpe))
    ...
    case q"$pack.lift[$t]($value)"                          => ScalarValueLift(value.toString, value.toString, value, q"null", inferQuat(q"$t".tpe))

...
import io.getquill.{ NamingStrategy, ReturnAction, TranspileContext }
...
object ExpandReturning {
  ...
  def applyMap(returning: ReturningAction)(f: (Ast, Statement) => String)(idiom: Idiom, naming: NamingStrategy, transpileContext: TranspileContext) = {
    idiom.idiomReturningCapability match {
      ...
      case ReturningMultipleFieldSupported =>
        val initialExpand = ExpandReturning(returning)(idiom, naming, transpileContext)
  ...
  def apply(returning: ReturningAction, renameAlias: Option[String] = None)(idiom: Idiom, naming: NamingStrategy, transpileContext: TranspileContext): List[(Ast, Statement)] = {
    ...
    val outputs = deTuplified.map(v => idiom.translate(v, dePropertized.quat, ExecutionType.Unknown, transpileContext))
    outputs.map {
      case (a, b, _) => (a, b)
    }
  }

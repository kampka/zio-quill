      ...
      statement.tokens.foldLeft(List.empty[Token]) {
        case (tokens, SetContainsToken(a, op, ScalarLiftToken(lift: ScalarQueryLift))) =>
          lift.value.asInstanceOf[Iterable[Any]].toList match {
            ...
            case values =>
              val liftings = values.map(v => ScalarLiftToken(ScalarValueLift(lift.name, lift.name, v, lift.encoder, lift.quat)))
...
object ReifyStatementWithInjectables {
  ...
  private def expandLiftings[T](statement: Statement, emptySetContainsToken: Token => Token, subBatch: List[T], injectables: collection.Map[String, T => ScalarLift]) = {
    ...
    def resolveInjectableValue(v: ScalarTagToken, value: T) = {
      val injectable = {
        ...
        injectables.get(v.tag.uid) match {
          ...
          case None        => throw new IllegalArgumentException(s"No insert-values entity found for the id: ${v.tag.uid}. Existing injectable values are: ${injectables}")
        }
      }

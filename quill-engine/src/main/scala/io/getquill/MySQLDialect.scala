...
trait MySQLDialect
  ...
  implicit def conflictTokenizer(implicit astTokenizer: Tokenizer[Ast], strategy: NamingStrategy, transpileContext: TranspileContext): Tokenizer[OnConflict] = {
    ...
    def tokenizer(implicit astTokenizer: Tokenizer[Ast]) =
      Tokenizer[OnConflict] {
        ...
        case OnConflict(i: io.getquill.ast.Action, NoTarget, Ignore) =>
          actionTokenizer(insertIgnoreTokenizer)(actionAstTokenizer, strategy, transpileContext).token(i)
    ...
    val customAstTokenizer =
      Tokenizer.withFallback[Ast](MySQLDialect.this.astTokenizer(_, strategy, transpileContext)) {
